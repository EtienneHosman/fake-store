  ...
  <div class="relative inline-block text-left">
    ...
    <div v-if="extended" class="origin-top-right absolute right-0 max-w-md w-96 top-12 bg-white rounded-lg md:max-w-lg border border-gray-400">
      ...
      <div class="flex flex-col divide-y divide-gray-200 overflow-y-auto max-h-80">
        <div v-if="store.listLength" class="flex items-center py-4 px-6" v-for="product in store.list">
          <img class="w-16 h-16 object-contain rounded" :src="product.image" alt="Product Image">
          <div class="ml-3">
            <h3 class="text-gray-900 font-semibold">{{ product.title }}</h3>
            <p class="text-gray-700 mt-1">€{{product.price}}</p>
          </div>
          <button class="ml-auto py-2 px-4 bg-blue-500 hover:bg-blue-600 text-white rounded-lg" @click="store.removeFromList(product.id)">
